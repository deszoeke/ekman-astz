...
HPC nodes differ in CPU: the head node is znver2, the compute nodes Sandy Bridge, and they share `~/.julia`. So everything is precompiled for both with one `JULIA_CPU_TARGET`, set in `precompile_slurm.sh` (`run_slurm.sh` reads it from there). Run `./precompile_slurm.sh` on the head node after package or EkmanCommon changes. Use `--clean` after changing the target, because Pkg's up-to-date check ignores the CPU target and keeps stale caches. That produced "Precompiled image … not available with flags". `pmap` gives each worker process whole groups, so there are no shared accumulators and no locks. Use processes, not threads, because NetCDF-C/HDF5 are not thread-safe.
